...
open class AloeStackView: UIScrollView {
  ...
  /// The handler will be called when the stack view have action pull to refresh.
  open var pullRefreshHandler: (() -> Void)?
  ...
  @objc private func pullRefresh(_ sender: Any) {
      if let pullRefreshHandler = pullRefreshHandler {
          pullRefreshHandler()
      }
  }
  ...
  // MARK: - Private
  private lazy var newRefreshControl: UIRefreshControl = {
      let refreshControl = UIRefreshControl()
      refreshControl.addTarget(self, action:
          #selector(AloeStackView.pullRefresh(_:)),
  ...
  private func setUpSelf() {
    ...
    if #available(iOS 10.0, *) {
        self.refreshControl = self.newRefreshControl
    } else {
        self.addSubview(self.newRefreshControl)
    }
  }
